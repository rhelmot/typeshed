# Stubs for networkx.algorithms.shortest_paths.unweighted (Python 3)
#
# NOTE: This dynamically typed stub was automatically generated by stubgen.

from _typeshed import Incomplete

def single_source_shortest_path_length(G, source, cutoff: Incomplete | None = ...): ...
def single_target_shortest_path_length(G, target, cutoff: Incomplete | None = ...): ...
def all_pairs_shortest_path_length(G, cutoff: Incomplete | None = ...) -> None: ...
def bidirectional_shortest_path(G, source, target): ...
def single_source_shortest_path(G, source, cutoff: Incomplete | None = ...): ...
def single_target_shortest_path(G, target, cutoff: Incomplete | None = ...): ...
def all_pairs_shortest_path(G, cutoff: Incomplete | None = ...) -> None: ...
def predecessor(
    G, source, target: Incomplete | None = ..., cutoff: Incomplete | None = ..., return_seen: Incomplete | None = ...
): ...
